...
import { getIdFromToken, getTokenFromCookie } from "./getToken";
    ...
    const fetchFavorites = async () => {
      try {
        const fav = await axios.get(
          `${baseUrl()}/v1/favorite/userfavourites?user_id=${userId}`
        );
    ...
    const fetchOrders = async () => {
      try {
        const ord = await axios.get(
          `${baseUrl()}/v1/orders/getorders?user=${userId}`
        );
        setOrders(ord.data);
      } catch (err) {
            ...
            <form>
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setProfile({ ...profile, patronymic: e.target.value })
                  }
                  className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                />
              </div>
